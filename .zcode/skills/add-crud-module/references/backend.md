# Backend: add a CRUD resource (Go + Kratos + Ent + Wire)
...
**Edit (4):**
7. `backend/app/admin/service/internal/data/providers/wire_set.go` — add `data.New<Entity>Repo,`
8. `backend/app/admin/service/internal/service/providers/wire_set.go` — add `service.New<Entity>Service,`
9. `backend/app/admin/service/internal/server/rest_server.go` — add param + `Register<Entity>HTTPServer`
10. `backend/api/buf.gen.yaml` — **only if** you created a brand-new domain (not permission/dict/identity/...). Add a `go_package` override entry.

**Never hand-edit (regenerated):** `api/gen/go/**`, `internal/data/ent/**` (except `schema/`), `cmd/server/wire_gen.go`.
...
## Step 9 — Update Wire ProviderSets (two files)

- `backend/app/admin/service/internal/data/providers/wire_set.go` → append `data.New<Entity>Repo,`
- `backend/app/admin/service/internal/service/providers/wire_set.go` → append `service.New<Entity>Service,`

(Do not touch `server/providers/wire_set.go` — services flow in via `NewRestServer` params.)

## Step 10 — Regenerate Wire

```bash
cd backend/app/admin/service && make wire
```
Regenerates `cmd/server/wire_gen.go`. If wire fails, it's almost always a missing ProviderSet entry from Step 9 or a typo in the constructor signature.
...
1. **`gow` vs `make`.** The `gow` CLI (from `go-wind-toolkit`) is documented in `backend/AGENTS.md` as `gow api` / `gow ent` / `gow wire`, but the project's `Makefile` actually wires `make api/ent/wire/gen`. Prefer `make` targets — they are the source of truth for what really runs. `gow` is fine if installed, just don't assume targets exist.
...
6. **Never edit `api/gen/go/`, `internal/data/ent/` (except `schema/`), or `wire_gen.go`.** If a generated symbol is wrong, fix the source (proto / schema / wire_set) and regenerate.
